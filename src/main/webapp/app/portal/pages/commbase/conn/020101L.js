$(document).ready(function() {
	var dbTypeId = "020101L";
	
	var colModel = [
		{ index:'CHK', name:'CHK', label:"선택", width:60, align:"center", sortable:false, formatter: 'customRadio', fixed:true},
		{ index:'ADM_OBJ_ID', name: 'ADM_OBJ_ID', label: 'DB 명', width: 150, align:'left'},
	    { index:'OBJ_NM', name: 'OBJ_NM', label: 'DB 논리명', width: 150, align:'left', editable: true, editrules: {required: true, readonly: "readonly"},
      	...
      	{ index:'DBINST_NM', name: 'DBINST_NM', label: 'DB 인스턴스명', width: 150, align:'left', editable: true, editrules: {required: true, readonly: "readonly"},
		    formatter: "dynamicLink", 
	    	formatoptions: {
	    		onClick: function (rowid, irow, icol, cellvalue, e) {
	    			var rowData = $grid.jqGrid("getRowData", rowid);
	    			DE.ui.open.popup(
    					"view",
    					[rowData["DBINST_TYPE_ID"], rowData["DBINST_ID"]],
    					{
    						viewname:'common/metacore/objectInfoTab',
    						objTypeId:rowData["DBINST_TYPE_ID"],
    						objId:rowData["DBINST_ID"],
    						mode:'RO'
    					},
    					null
    				);
      	...
      	{ index:'JDBC_NM', name: 'JDBC_NM', label: 'JDBC 명', width: 150, align:'left', editable: true, editrules: {required: true, readonly: "readonly"},
		    formatter: "dynamicLink", 
	    	formatoptions: {
	    		onClick: function (rowid, irow, icol, cellvalue, e) {
	    			var rowData = $grid.jqGrid("getRowData", rowid);
	    			DE.ui.open.popup(
    					"view",
    					[rowData["JDBC_TYPE_ID"], rowData["JDBC_ID"]],
    					{
    						viewname:'common/metacore/objectInfoTab',
    						objTypeId:rowData["JDBC_TYPE_ID"],
    						objId:rowData["JDBC_ID"],
    						mode:'RO'
    					},
    					null
    				);
      	...
      	{ index:'DBINST_TYPE_ID', name: 'DBINST_TYPE_ID', label: 'DBINST_TYPE_ID', hidden:true},
      	{ index:'DBINST_ID', name: 'DBINST_ID', label: 'DBINST_ID', hidden:true}, 
      	{ index:'JDBC_TYPE_ID', name: 'JDBC_TYPE_ID', label: 'JDBC_TYPE_ID', hidden:true},
      	{ index:'JDBC_ID', name: 'JDBC_ID', label: 'JDBC_ID', hidden:true}
    ];
	...
	$("button#btnSearch").on("click", function(e) {
		var postData = {
			"queryId":"portal.commbase.findDatabase",
			"objTypeId":dbTypeId,
