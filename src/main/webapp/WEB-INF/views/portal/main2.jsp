...
.main_pop .template_frm {display:none !important;}
.main_pop .template_val {display:block !important;}
</style>
	...
	<div class="row">
		<div class="col-sm-12 col-md-12" style="margin-top:80px;">
			<div class="main_contents_Area">
				...
				<li onclick="goPage('mymenu','','portal/report/bookmarkReportList')">
...
<!--  레이어 팝업 -->
<div id="pop" class="main_pop" style="display:none">
 	<div style="height:370px; text-align:left;">
   	 	...
   	 	<div style="width:100%; padding:20px;" class="notiDiv">     	 		
   	 	</div>   	 	
  	</div>
  	<div style="width:100%; background:#000; height:26px;">
  		<div style="color:#fff;    width: 50%; float: left; text-align: left; padding: 3px 0 0 10px;">
  			...
  			<span style="vertical-align: text-bottom; font-size: 0.9em;">오늘 하루 안보기</span>
		</div>
    	<div class="close" style="float:right; width:40%; text-align:right; color:#fff !important; padding:3px 6px 0 0; cursor:pointer; font-size:24px;">×</div>
  	</div>
</div>
...
function noticePop(){
	var param = {"_search":false,"nd":1566802058509,"rows":10,"page":1,"sidx":"","sord":"asc",
			"queryId":"portal.notice.getMainPopNotiList"}
	$.ajax({ 
	    url:'jqgrid/list', //request 보낼 서버의 경로
	    type:'post', // 메소드(get, post, put 등)
	    contentType: 'application/json; charset=utf-8',
	    ...
	    success: function(data) {
	    	...
	    	if(popList != "") {
		    	for(let i in popList){
					...
					let top = 10 + 10*i;
					let left = 100 + 300*i
					popup.attr("id","pop"+d.NTC_ID);
			    	let notiHtml = "";		
			    	notiHtml += "<div style=\"font-weight:bold; font-size:14px; color:#333;\">" + d["NTC_NM"] + "</div>";
			    	notiHtml += "<div class=\"Grid_Area_main\">";
			    	notiHtml += "	<table class=\"table_main\">";
			    	notiHtml += "		<tbody>";
			    	notiHtml += "			<tr>";
			    	notiHtml += "				<th>작성자</th>";
			    	notiHtml += "				<td>" + d["CRETR_ID"] + "</td>";
			    	notiHtml += "			</tr>";
			    	notiHtml += "			<tr>";
			    	notiHtml += "				<th>기간</th>";
			    	notiHtml += "				<td>" + d["NTC_ST_DATE"] + " ~ " + d["NTC_ED_DATE"] + "</td>";
			    	notiHtml += "			</tr>";
			    	notiHtml += "			<tr>";
			    	notiHtml += "				<td colspan=\"2\" class=\"pop_contents\">";
			    	notiHtml += "				<span style=\"float:left; width:100%; height:160px; overflow:auto;\">";	    	
			    	notiHtml += 							d["NTC_SBST"];	    	
			    	notiHtml += "				</span></td>";
			    	notiHtml += "			</tr>";
			    	notiHtml += "		</tbody>";
			    	...
			    	$("body").append(popup);
			    	popup.find(".notiDiv").html(notiHtml);
			    	popup.show();
		    	} 
