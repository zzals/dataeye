...
$(function() {
  ...
  $('#edate').datepicker('setDate', 15);
 	...
 	function goUpdateForm() {
 		...
 		var post = {
				"viewId": "portal/faq/boardUpdateForm",
				"menuId": "",
				"data":"{\"brd_id\":\"" + data["brd_id"] + "\",\"brd_type_id\":\"faq\"}"
			};
 	 ...
 	 function doDeleteProc() {
		...
		if(cf) {	 		
	 		     $.ajax({
	                url: 'board/doDeleteProc?brd_id=' + $("#brd_id").val() + '&brd_type_id=faq&boardAuthKey=' + encodeURIComponent('<%=request.getAttribute("boardAuthKey")%>') ,                
	                data: '',
	 ...
	 function viewReply(){
		 var param = {"_search":false,"nd":1566802058509,"rows":10,"page":1,"sidx":"","sord":"asc","queryId":"portal.board.getBoardReplyList","brd_type_id":"faq","brd_id":$("#brd_id").val()}
			$.ajax({ 
...
$(document).ready(function() {
	...
	var param = {"_search":false,"nd":1566802058509,"rows":10,"page":1,"sidx":"","sord":"asc","queryId":"portal.board.getBoardDetail","brd_type_id":"faq","brd_id":data["brd_id"]}
	$.ajax({ 
	    url:'jqgrid/list', //request 보낼 서버의 경로
	    type:'post', // 메소드(get, post, put 등)
	    contentType: 'application/json; charset=utf-8',
	    ...
	    success: function(data) {
	    	...
	    	if(data["rows"][0]["FILE_NM"] !=null) {
		     ...
		     $.ajax({
               url: 'board/doReadCntProc?brd_id=' + $("#brd_id").val() + '&brd_type_id=faq&boardAuthKey=' + encodeURIComponent('<%=request.getAttribute("boardAuthKey")%>') ,                
               data: '',
               contentType: 'application/json; charset=utf-8',
               type: 'POST',
               success: function(result){
