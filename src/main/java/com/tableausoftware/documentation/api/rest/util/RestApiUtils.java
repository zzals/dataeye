...
public class RestApiUtils {
    ...
    private enum Operation {
    	...
    	GROUP_LIST(getApiUriBuilder().path("sites/{siteId}/groups"));
    ...
    private static UriBuilder getApiUriBuilder() {
        return UriBuilder.fromPath(m_properties.getProperty("server.host") + "/api/3.8");
    }
    /**
     ...
     * @throws SAXException 
     */
    private static void initialize() throws IOException, JAXBException, SAXException {
    ...
    public GroupListType invokeQueryGroups(TableauCredentialsType credential, String siteId) {
        ...
        m_logger.info(String.format("Querying gropus on site '%s'.", siteId));

        String url = Operation.GROUP_LIST.getUrl(siteId);
