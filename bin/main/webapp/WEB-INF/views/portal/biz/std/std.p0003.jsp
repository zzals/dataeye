				...
				<div class="collapse navbar-collapse">
					<div class="navbar-form navbar-right de-obj-btn ">
						<c:if test="${AUTH.getRAuth() == 'Y'}">
							<a id="wordUploadTplDown" class="btn btn-default">템플릿 다운로드</a>
							<a id="wordUploadTplUpload" class="btn btn-default">업로드</a>
							<a type="button" id="wordVerifyChkAction" class="btn btn-default">단어검증</a>
						</c:if>
						<c:if test="${AUTH.getCAuth() == 'Y'}">
							<a type="submit" id="batchSaveAction" class="btn btn-default">일괄등록</a>
						</c:if>
					</div>
				</div>
			</div>
...
<script src="<spring:url value="/app/portal/pages/biz/std/std.p0003.js"/>"></script>
</body>
</html>